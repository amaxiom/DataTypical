...
- **Added `formative_method='exact'`**: a closed form for the archetypal
  formative values, with no sampling, no seed dependence, and O(n log n) cost.
  Opt-in, so the default is unchanged.
